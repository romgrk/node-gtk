...
function generateSource(namespaces) {
  ...
  return removeTrailingSpaces(unindent(`
    ...
    /*
     * Initialize scaled_font.
     */

    ScaledFont::ScaledFont(${base.type}* data) : ObjectWrap() {
      _data = data;
    }
...
${options.isBase ? `
      ${namespaces.filter(ns => ns.name !== 'ScaledFont').map(ns =>
...
function generateNewMethod(options) {
  ...
  return `
    ...
    NAN_METHOD(${options.name}::New) {
      ...
      ${options.name}* scaled_font = new ${options.name}(data);
      scaled_font->Wrap(info.This());
