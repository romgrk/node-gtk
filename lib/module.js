...
// Listeners invoked after a namespace is freshly loaded. Used by register-class.js
// to flush deferred class registrations once their parent namespace is available.
const requireListeners = []
...
// Non-enumerable so it stays internal (index.js spreads the public exports).
Object.defineProperty(module.exports, 'onRequire', {
  value: onRequire,
  enumerable: false,
})

function onRequire(fn) {
  requireListeners.push(fn)
}
...
function giRequire(ns, version) {
  ...
  // Notify listeners that a new namespace finished loading (parent GTypes for
  // deferred class registrations may now be available).
  requireListeners.forEach(fn => fn(ns))
