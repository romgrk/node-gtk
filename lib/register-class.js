...
function registerClass(klass) {
  ...
  if (parentGtype === GObject.TYPE_INVALID)
    throw new Error(`Parent class not registered registered: ${parent.name}`)

  console.log(klass)

  // Register the class with the type system
  const klassGtype = internal.RegisterClass(name, klass, parent.name, parent)
  ...
  console.log({ klassGtype })
}

// Helpers

function setupVirtualFunctions(klass, klassGtype, parentGtype) {
  // FIXME: handle clase where parent isn't a GIR object
  // and assert it's a GObject
  ...
  Object.getOwnPropertyNames(klass.prototype).forEach(key => {
    ...
    const [vfuncInfo, definedByParent] = findVFunc(klassGtype, parentInfo, nativeName)
    console.log(['prop', nativeName, vfuncInfo, definedByParent])
...
function findVFunc(gtype, parentInfo, name) {
  let [vfuncInfo, definedByParent] = findVFuncOnParents(parentInfo, name)
  if (!vfuncInfo) {
    vfuncInfo = findVFuncOnInterfaces(gtype, name)
    definedByParent = false
  }
  return [vfuncInfo, definedByParent]
}
...
function findInfoByGtype(gtype) {
  return GI.Repository_find_by_gtype.call(GI.Repository_get_default(), gtype)
}
